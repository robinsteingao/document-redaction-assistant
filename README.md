...
- 静态桌面壳可绑定本地服务 URL，通过浏览器页面调用本地服务生成脱敏上传包。
...
- 复核工作区展示影响等级和评价影响提示，桌面壳提供客户自定义脱敏决策 JSON 输入框。
...
- M24 起如粘贴客户自定义脱敏决策，系统只表示“客户提交过决策”；只有显式勾选“确认接受评价降级风险”时，才会在门禁摘要中记录 `customer_confirmed=true`。
- 点击“开始生成脱敏包”，等待处理进度完成。
